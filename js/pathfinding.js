...
(function () {
    ...
    function buildRailGraph(railWays, railNodes, excludedColors = new Set()) {
        const graph = new Map();
        railWays.forEach(way => {
            if (way.color && excludedColors.has(way.color)) return;
            for (let i = 1; i < way.nodes.length; i++) {
                ...
                const edgeBase = { weight, line: way.routeId || way.id, transfer: false };
    ...
    function shortestPath(startId, endId, railGraph, heuristic = () => 0, algorithm = 'Dijkstra') {
        const startedAt = performance.now();
            ...
            (railGraph.get(current.id) || []).forEach(edge => {
                ...
                if (!distances.has(nextKey) || candidate < distances.get(nextKey)) {
                    ...
                    open.push({
                        ...
                        key: nextKey,
                        cost: candidate,
                        priority: candidate + heuristic(edge.to)
                    });
                }
            });
        }
    ...
    function octileHeuristic(railNodes, endId) {
        const target = railNodes.get(endId);
        return nodeId => {
            const node = railNodes.get(nodeId);
            if (!node || !target) return 0;
            ...
            const diagonal = Math.SQRT2 * Math.min(dx, dy);
            return (straight + diagonal) / METERS_PER_MINUTE;
        };
    }
    ...
    function compareAlgorithms(startId, endId, railNodes, railGraph) {
        ...
        const results = [
            ...
            shortestPath(startId, endId, railGraph, octileHeuristic(railNodes, endId), 'A* Octile')
        ];
    ...
    function findRailPath(startStationId, endStationId, stations, railNodes, railGraph, algorithm = 'dijkstra') {
        ...
        const search =
            ...
            algorithm === 'astar-octile' ? shortestPath(startNode.nodeId, endNode.nodeId, railGraph, octileHeuristic(railNodes, endNode.nodeId), 'A* Octile') :
